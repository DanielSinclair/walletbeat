	...
	for (const ratedWallet of Object.values(allRatedWallets)) {
		...
		for (const { tree, variantName } of trees) {
			mapNonExemptAttributeGroupsInTree(tree, (attrGroup, evalGroup) => {
				// eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- Safe because all attribute groups type parameters extend OutcomeMetadataSet.
				const genericAttrGroup = attrGroup as unknown as AttributeGroup<OutcomeMetadataSet>
