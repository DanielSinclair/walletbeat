...
export function evaluateAttributes(
	...
	const evalAttr = <OM extends OutcomeMetadata>(attr: Attribute<OM>): EvaluatedAttribute<OM> => {
		const ctx = EvaluationContext.create<OM>(attr, features)
...
export function aggregateAttributes(perVariant: AtLeastOneVariant<EvaluationTree>): EvaluationTree {
	const attr = <OM extends OutcomeMetadata>(
		getter: (tree: EvaluationTree) => EvaluatedAttribute<OM>,
	): EvaluatedAttribute<OM> => {
...
export function mapNonExemptGroupAttributes<T, Vs extends ValueSet>(
	...
	fn: <OM extends OutcomeMetadata>(evalAttr: EvaluatedAttribute<OM>, index: number) => T,
): T[] {
	return Object.values(evalGroup)
		.filter(
			<OM extends OutcomeMetadata>(evalAttr: EvaluatedAttribute<OM>): boolean =>
				evalAttr.evaluation.outcome.rating !== Rating.EXEMPT,
		)
		.map(fn)
}
...
export function mapAttributesGetter(
	...
	fn: <OM extends OutcomeMetadata>(
		getter: (evalTree: EvaluationTree) => EvaluatedAttribute<OM> | undefined,
	) => void,
): void {
	for (const groupName of Object.keys(templateTree)) {
		for (const attrName of Object.keys(templateTree[groupName])) {
			fn(
				<OM extends OutcomeMetadata>(
					evalTree: EvaluationTree,
				): EvaluatedAttribute<OM> | undefined =>
					...
					(evalTree[groupName] as any)[attrName] as EvaluatedAttribute<OM>,
			)
		}
	}
}
 ...
 */
export function getEvaluationFromOtherTree<OM extends OutcomeMetadata>(
	evalAttr: EvaluatedAttribute<OM>,
	...
	const otherEvalAttr = mapNonExemptAttributeGroupsInTree(
		...
		<Vs extends ValueSet>(
			...
			if (Object.hasOwn(evalGroup, evalAttr.attribute.id)) {
				...
				return evalGroup[evalAttr.attribute.id] as unknown as EvaluatedAttribute<OM>
			}
...
export function getAttributeFromTree<OM extends OutcomeMetadata>(
	tree: EvaluationTree,
	attribute: Attribute<OM>,
): EvaluatedAttribute<OM> | null {
		...
		<Vs extends ValueSet>(
			...
			for (const evalAttr of evaluatedAttributes(evalGroup)) {
				if (evalAttr.attribute.id === attribute.id) {
					...
					return evalAttr as unknown as EvaluatedAttribute<OM>
				}
			}
